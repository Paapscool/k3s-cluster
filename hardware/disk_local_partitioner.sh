...
sudo umount /dev/$HARDWARE_DISK_NAME
...
## Format partitions
yes n | sudo mkfs.vfat -F 32 -n boot /dev/$HARDWARE_DISK_NAME$EFI_NUM_PART
yes n | sudo mkswap -L swap /dev/$HARDWARE_DISK_NAME$SWAP_NUM_PART
...
sudo swapon /dev/$HARDWARE_DISK_NAME$SWAP_NUM_PART
sudo swapoff /dev/$HARDWARE_RESCUE_DISK_NAME$SYSTEM_SWAP_NUM_PART

## improve script utils to mount partitions
echo "" >> /utils_mount.sh
echo "sudo swapon /dev/$HARDWARE_DISK_NAME$SWAP_NUM_PART" >> /utils_mount.sh
echo "sudo swapoff /dev/$HARDWARE_RESCUE_DISK_NAME$SYSTEM_SWAP_NUM_PART" >> /utils_mount.sh
...
SWAP_UUID_PART=$(lsblk -o name,uuid | grep -w $HARDWARE_DISK_NAME$SWAP_NUM_PART | sed -e 's/.*'$HARDWARE_DISK_NAME$SWAP_NUM_PART' //g')
...
echo "UUID=$EFI_UUID_PART /boot/efi vfat noatime,umask=0077 0 1" >> $FSTAB_PATH
echo "UUID=$SWAP_UUID_PART none swap sw 0 0" >> $FSTAB_PATH
...
INITRAMFS_RESUME_PATH=./resources/resume
echo "RESUME=UUID=$SWAP_UUID_PART" > $INITRAMFS_RESUME_PATH
